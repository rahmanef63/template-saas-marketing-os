...
const SUGGESTIONS = [
  ...
  "Which pricing plan fits my team?",
  "How do I start a trial?",
];

export function AiChatFab({ brand = "us" }: { brand?: string }) {
  ...
  const [msgs, setMsgs] = React.useState<Msg[]>([
    {
      role: "assistant",
      text: `Hi 👋 I'm the ${brand} assistant. Ask me anything about features, pricing, or getting started.`,
    },
  ]);
  ...
  async function send(q: string) {
    ...
    try {
      ...
      setMsgs((m) => [
        ...
        {
          role: "assistant",
          text: `Failed to reach the assistant: ${(e as Error).message}`,
          notice: true,
        },
      ]);
    } finally {
        ...
        <Card className="fixed bottom-20 right-5 z-40 flex h-[26rem] w-[20rem] flex-col overflow-hidden border-border/60 shadow-xl sm:w-[22rem]">
          <div className="flex items-center justify-between border-b bg-muted/40 px-4 py-3">
            <div className="flex items-center gap-2 text-sm font-semibold">
              <Sparkles className="size-4 text-primary" /> {brand} assistant
            </div>
            <button
          ...
          >
            <Input
              ...
              disabled={pending}
            />
            <Button type="submit" size="icon" className="h-9 w-9 shrink-0" aria-label="Send" disabled={pending}>
