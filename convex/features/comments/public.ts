 ...
 *
 * The slice ships `_listForTarget` as an internalQuery (default-deny). For this
 * SaaS marketing site, blog post detail pages are publicly readable, so we
 * expose a thin public query that proxies the internal one and maps the raw
 * `comment_threads` docs into the slice's `Comment` DTO shape consumed by
